...
public class ChangeModeController {
    ...
    public void setTheme(Activity ctx) {
        ...
        if (mTheme == 0) {
            mTheme = ThemeModel.RED.getTheme();
            ctx.setTheme(mTheme); //如果没有保存过系统主题，则默认使用红色主题
        } else {
            ThemeModel themeModel = ThemeModel.valueOf(mTheme);
            ctx.setTheme(themeModel.getTheme());
        }
    }
     ...
     * 更改主题
     */
    public void changeNight(Activity ctx, ThemeModel themeModel) {
        ctx.setTheme(themeModel.getTheme());
        showAnimation(ctx);
        refreshUI(ctx);
        SharedPref.get().put(PRE_THEME_MODEL, themeModel.getTheme());
    }
        ...
        public int applyValue(Context context, String resName, int theme) {
            ...
            if (!not) {
                ...
                if (!endThemeStr.equals(resName)) {
                    not = true;
                    resName = resName.replace(endThemeStr, ThemeModel.valueOf(theme).getThemeStr());
                }
            }
